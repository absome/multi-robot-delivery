...
class DetectObstacle():
    def __init__(self, detect_max_dist=10, angle_res_rad=np.pi/180):
        self.__max_dist = detect_max_dist # max sensing distance
        self.__res_rad = angle_res_rad # angular gap between sensing rays
        self.__sens_num = round(2*np.pi/angle_res_rad) # number of rays
    ...
    def register_obstacle_bounded(self, vertices):
        """
        Takes a single polygon formed as a numpy arrar that represents an
        obstacle. Stores obstacle as a line segment
        
        Args:
            vertices::[np.array]
                Nx3 Numpy array with vertex locations that form an obstacle
                shape
        """
        # store list of vertices that construct the obstacle into self.__line_segment
        # expect the vertices to be numpy array N x 3 
        assert np.allclose(vertices[0, :2], vertices[-1, :2]), (
            "The last vertex should be the same as the first in (x,y)."
        )
    ...
    def get_sensing_data(self, posx, posy, theta_rad):
        """
        Computes detected obstacle using interesction of sensing line-segment
        and obstacle's line-segment, based on https://en.wikipedia.org/wiki/Line%E2%80%93line_intersection.
        
        Args:
            posx::[float]
                x position of sensor in world coordinates
            posy::[float]
                y position of sensor in world coordinates
            theta_rad::[float]
                orienation of the sensor/robot
        Returns:
            sensing_data::[np.array]
